...
//  }
  ...
  size_undis = cv::Size2i(
                 std::abs(std::round(std::fmax(cns_ud[1].x, cns_ud[2].x)))
                  + std::abs(std::round(std::fmin(cns_ud[0].x, cns_ud[3].x))),
                 std::abs(std::round(std::fmax(cns_ud[2].y, cns_ud[3].y)))
                  + std::abs(std::round(std::fmin(cns_ud[0].y, cns_ud[1].y)))
      );
    ...
    bool _force_dim)
{
  ...
  else if (in_res.width < proj_res.width || in_res.height < proj_res.height)
  {
    ...
    out_res = cv::Size2i(
                   std::abs(std::round(std::fmax(cns_d[1].x, cns_d[2].x)))
                    - x_min,
                   std::abs(std::round(std::fmax(cns_d[2].y, cns_d[3].y)))
                    - y_min
        );
  }
      ...
      for (uint row = 0; row < out_res.height; ++row)
      {
            ...
            ref_h < (int) in_res.height-1)
        {
          img_out.at<uchar>(row,col) =
              pix_diff.x * (pix_diff.y*_image_in.at<uchar>(ref_h+1, ref_w+1)
                            + (1-pix_diff.y)*_image_in.at<uchar>(ref_h, ref_w+1))
              + (1-pix_diff.x) * (pix_diff.y*_image_in.at<uchar>(ref_h+1, ref_w)
                                  + (1-pix_diff.y)*_image_in.at<uchar>(ref_h, ref_w));
        } // if
        else
        {
      ...
      for (uint row = 0; row < out_res.height; ++row)
      {
            ...
            ref_h < (int) in_res.height-1)
        {
          img_out.at<cv::Vec3b>(row,col) =
              pix_diff.x * (pix_diff.y*_image_in.at<cv::Vec3b>(ref_h+1, ref_w+1)
                            + (1-pix_diff.y)*_image_in.at<cv::Vec3b>(ref_h, ref_w+1))
              + (1-pix_diff.x) * (pix_diff.y*_image_in.at<cv::Vec3b>(ref_h+1, ref_w)
                                  + (1-pix_diff.y)*_image_in.at<cv::Vec3b>(ref_h, ref_w));
        } // if
        else
        {
